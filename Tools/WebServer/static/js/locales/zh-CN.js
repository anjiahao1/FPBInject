...
window.i18nResources['zh-CN'] = {
  ...
  tutorial: {
    ...
    connection_desc: '选择设备的串口和波特率。你可以稍后在配置面板中修改。',
    connection_refresh: '刷新端口',
    connection_test: '测试连接',
    connection_success: '连接成功！',
    connection_fail: '连接失败。你可以跳过稍后配置。',
    connection_skip_hint: '没有设备？跳过此步骤即可。',

    project_title: '项目配置',
    project_desc: '设置 ELF 固件文件和编译数据库路径，用于符号解析和补丁编译。',

    inject_title: '注入配置',
    inject_desc: '配置补丁注入模式和文件监视目录。',

    quickcmd_title: '快捷指令',
    quickcmd_desc: '快捷指令可以一键发送串口命令或执行命令序列（宏）。',
    quickcmd_feature_single: '单条命令',
    quickcmd_feature_single_desc: '即时发送一条串口命令。',
    quickcmd_feature_macro: '组合命令',
    quickcmd_feature_macro_desc: '按顺序执行多条命令，支持延时。',

    complete_title: '设置完成！',
    complete_desc: '你可以随时在侧边栏配置面板中修改这些设置。',
